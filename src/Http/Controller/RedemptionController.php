...
use Xoxoday\Storefront\Models\Xoredemption;
use Xoxoday\Storefront\Models\User;
use Xoxoday\Storefront\Models\Xostate;
use Xoxoday\Storefront\Models\XoloyaltyPoint;
...
{
    /*
     * Function to show the redemption page
     */
    public function index()
    {   
        
        $products =  Xoproduct::get();
        $total_points = 0;
        $mobile = '';
        $user_bit = 0;
        $name = '';
        ...
        if (Auth::check()) {
            $user = Auth::user();
            $total_points = XoloyaltyPoint::where('user_id', $user['id'])->sum('points');
            $first_name_array = explode(' ',$user->name);
            $first_name = $first_name_array[0];
            $name = $user->name;
            $mobile = $user->mobile;
            $user_bit = 1;
            $inprogress_product = Xoredemption::where('user_id', $user['id'])->where('status','0')->get()->pluck('product')->toArray();
    ...
    public function verifyLogin(Request $request)
    {
        ...
        if (isset($request->mobile) && isset($request->otp) && isset($request->product_id)) {
            ...
            if (Auth::check()) {
                $otp = Config('xostorefront.default_otp');
            } else {
                $otp = $request->otp;
            }
            ...
            if (Auth::attempt(array('mobile' => $mobile, 'password' => $otp))) {
                ...
                $user = Auth::user();
                $product = Xoproduct::where('id', $product_id)->first();
                $inprogress_product = Xoredemption::where('user_id', $user['id'])->where('status','0')->get()->pluck('product')->toArray();
    ...
    /*
     * Function to  redeeem the storefront products
     */
    public function redeemPoints(Request $request)
    {
        ...
        if ($user_id) {
            $product = Xoproduct::where('id', $product_id)->first();
            $total_points = XoloyaltyPoint::where('user_id', $user_id['id'])->sum('points');
            if ($total_points >= $product['points']) {
                try {
                    $redemption_create = Xoredemption::create([
                        'user_id' => $user_id['id'],
                        'product' => $product_id,
    ...
    private function sendSMS($mobile, $otp)
    {
        ...
        return $sms->postSmsRequest($country, $mobile,Config('xostorefront.sms_message_otp'), $variables);
    }
    ...
    public function addLoyaltyPoints($user_id, $reference_id, $points, $reason, $redemption_id)
    {
        try {
            $loyalty_points_entry = XoloyaltyPoint::create([
                'user_id' => $user_id,
